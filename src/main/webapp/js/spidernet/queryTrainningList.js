			...
			.ajax({
				...
				success : function(result) {
					...
					for (var i = 0; i < result.data.length; i++) {
						...
						/*
						 * var td1 = $("<td>" + result.data[i].trainningId + "</td>");
						 */
						...
						var td6 = $("<td><a class='btn btn-primary btn-sm' href='javascript:void(0);' onclick=detailTrainningplan('"
			...
			.ajax({
				...
				success : function(result) {
					...
					for (var i = 0; i < result.length; i++) {
						var tr = $("<tr></tr>");
						tr.appendTo(tbody);
						var td1 = $("<td>" + result[i].parentTrainingName
								+ "</td>");
						var td2 = $("<td>" + result[i].childTrainName + "</td>");
						var td3 = $("<td>" + result[i].trainTime + "</td>");
						var td4 = $("<td>" + result[i].trainRoom + "</td>");
						var td5 = $("<td>" + result[i].participants + "</td>");
						var td6 = $("<td> <a class='btn btn-info btn-xs' href='javascript:void(0);' onclick=updateDetailTrainningplanMode('"
...
function saveTrainningdetailPlan() {
	...
	if (parentTrainingName == "") {
		alert("can not set empty parent knowledge point");
		return false;
	}
	if (childTrainingName == "") {
		alert("can not set empty child knowledge point");
		return false;
	}
	if (trainningTime == "") {
		alert("can not set empty training time");
		return false;
	}
	if (trainingRoom == "") {
		alert("can not set empty address");
		return false;
	}
	if (participantsNumber == "") {
		alert("can not set empty participants number");
		return false;
	}
...
function updateTrainningdetailPlan() {
	...
	if (updatenowledgePoint == "") {
		alert("can not set empty parent knowledge point");
		return false;
	}
	if (updateChildKnowledge == "") {
		alert("can not set empty child knowledge point");
		return false;
	}
	if (updateDetailtrainningTime == "") {
		alert("can not set empty training time");
		return false;
	}
	if (updatelocation == "") {
